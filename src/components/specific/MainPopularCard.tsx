...
const MainPopularCard: React.FC<MainPopularCardProps> = ({ name, link, home, introduction }) => {
  return (
    <div className='relative my-2 flex items-center justify-center'>
      <img className='rounded-sm' src={home} alt={name} />
      <div className='absolute left-0 top-0 flex h-full w-full flex-col justify-between p-2 md:p-3'>
        <div className='flex justify-between'>
          <div className='truncate text-[10px] font-bold sm:text-xs md:text-sm lg:text-base xl:text-lg'>{name}</div>
        ...
        <div className='flex items-center sm:justify-end md:justify-between md:gap-1 lg:gap-2'>
          <p className='hidden grow truncate md:block md:text-[10px] lg:text-xs'>{introduction.description}</p>
          <Link
